...
**Exceção mantida:** 2+ consultas no MESMO telefone e MESMO dia (família) — o toque não diz de quem é, então a Ana pergunta **QUAL** (não mais "tem certeza?": a intenção já está dada). Texto livre ("quero desmarcar") também continua pela Ana. Rótulos em `BOTOES_LEMBRETE = ["Confirmo","Desmarcar","Remarcar"]`; template `lembrete_consulta_botoes` (settings `lembrete_template`), no ar desde 16/08.
