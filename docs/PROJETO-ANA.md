...
- A lista tem **uma opção por cirurgia, sem tipo nem LIO** ("Cirurgia refrativa",
  "Cirurgia de catarata"): na saída da consulta a técnica e a lente muitas vezes
  ainda não estão definidas. O valor automático é um **piso** (R$ 5.990 e R$ 5.000)
  e a tela diz "no mínimo" — quem quiser precisão digita o valor, ou escreve o tipo
  por extenso, que a tabela do servidor reconhece ("catarata trifocal tórica" →
  R$ 18.200).
- A indicação grava o **convênio**, copiado do agendamento: em paciente de plano o
  valor nasce vazio (a cobertura é caso a caso) e a Ana é proibida de citar valor
  particular a ele. O total do funil soma só o que o paciente paga.
